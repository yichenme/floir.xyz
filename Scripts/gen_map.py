...
def _sync_collision_editor(gm, GW, GH):
    # Downsample the global collision mask (GW x GH, WORLD_U_PER_PX world u/px)
    # to the editor's 25-world-u grid and patch collision-editor.html's B64_INIT.
    import base64
    import re
    EDITOR_UNIT = 25
    UW = int(GW * WORLD_U_PER_PX) // EDITOR_UNIT
    UH = int(GH * WORLD_U_PER_PX) // EDITOR_UNIT
    buf = bytearray((UW * UH + 7) // 8)
    half = EDITOR_UNIT / 2.0
    for uy in range(UH):
        py = int((uy * EDITOR_UNIT + half) / WORLD_U_PER_PX)
        if py >= GH:
            py = GH - 1
        base = py * GW
        rowbase = uy * UW
        for ux in range(UW):
            px = int((ux * EDITOR_UNIT + half) / WORLD_U_PER_PX)
            if px >= GW:
                px = GW - 1
            if gm[base + px]:
                idx = rowbase + ux
...
def write_tilemap_header(layers, objgroups, W, H, terrain):
    ...
    lines = [
        ...
        f'//    one union mask at SUB sub-cells/cell ({WORLD_U_PER_PX:g} world u/px). Each cell is',
        ...
        f'    inline constexpr float    COLL_UNIT = CELL_SIZE / SUB;   // {WORLD_U_PER_PX:g} world u/px',
    ...
    lines += ['    };', '',
        ...
        '            if (c<0||r<0||c>=(int)GRID_W||r>=(int)GRID_H) return true;',
        '            if (CELL_MASK[c + r*(int)GRID_W] != 0) return true;',
        '        }',
        '        return false;',
        '    }', '',
        ...
        '            for (int uy=cy0; uy<=cy1; uy+=st) for (int ux=cx0; ux<=cx1; ux+=st) {',
        '                if (!_usolid(ux,uy)) continue;',
        '                float ax0=ux*U, ay0=uy*U, ax1=ax0+Ub, ay1=ay0+Ub;',
        ...
        '                };',
        '                if (!_usolid(ux-st,uy)) face(-1,0, ax0,ay0, ax0,ay1);',
        '                if (!_usolid(ux+st,uy)) face( 1,0, ax1,ay0, ax1,ay1);',
        '                if (!_usolid(ux,uy-st)) face(0,-1, ax0,ay0, ax1,ay0);',
        '                if (!_usolid(ux,uy+st)) face(0, 1, ax0,ay1, ax1,ay1);',
        '            }',
        '            if (!found) break;',
        '            x=bx; y=by;',
        '        }',
